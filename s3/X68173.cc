...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
  ...
  if (imax != -1 and est.te_nota()) {
      ...
      else if (est.consultar_DNI()<vest[imax].consultar_DNI()) {
          if (est.consultar_nota()==vest[imax].consultar_nota()) imax=i+1;
          else ++imax;
      }
  }
  else if (imax == -1 and est.te_nota()) imax =i+1;
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  if (i==imax) {
    ...
    else if (imax > i and est.consultar_nota() >= vest[imax].consultar_nota()) imax=i;
    }
...
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est) {
  ...
  if (i-1==imax) {
    ...
    if (est.consultar_nota()>vest[imax].consultar_nota()) imax=i-1;
    else if (imax > i-1 and est.consultar_nota() >= vest[imax].consultar_nota()) imax=i-1;
    }
